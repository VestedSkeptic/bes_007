...
(margin?num(this,"margin"+tl)+num(this,"margin"+br):0);};});})();function vote_onItem_03(elem,object_id,content_type,ajaxProcessingUrl,currentVote,thisVote,vote_1,vote_2,vote_3){var divParent=$(elem).parent();var upChild=divParent.children().filter("div:first");var downChild=divParent.children().filter("div:last");var middleChild=divParent.children().filter("div:eq(1)");if(thisVote=='up'){if(upChild.attr("class")=="arrow upmod"){upChild.attr("class","arrow up");middleChild.html(vote_2);}
else{upChild.attr("class","arrow upmod");middleChild.html(vote_1);}
downChild.attr("class","arrow down");}
else{upChild.attr("class","arrow up");if(downChild.attr("class")=="arrow downmod"){downChild.attr("class","arrow down");middleChild.html(vote_2);}
else{downChild.attr("class","arrow downmod");middleChild.html(vote_3);}}
var dataDict={};dataDict['object_id']=object_id;dataDict['content_type']=content_type;$.post(ajaxProcessingUrl,dataDict);return false;};function toggle_subscription_03(elem,object_id,content_type,ajaxProcessingUrl){if($(elem).html()=="Subscribe"){$(elem).html("Un-Subscribe");}
else{$(elem).html("Subscribe");};var dataDict={};dataDict['object_id']=object_id;dataDict['content_type']=content_type;$.post(ajaxProcessingUrl,dataDict);return false;};function toggle_03(elem,key,callback){var self=$(elem).parent().andSelf().filter("."+key);var sibling=self.toggleClass("inactive").siblings().toggleClass("inactive").get(0);if(callback)callback(elem);return false;};function extract_thread_id(ff_name){var split1=ff_name.split(":");var split2=split1[1].split("-",1);return split2;};function cancel_edit_05(elem){var formSpan=$(elem).parents("span[id=formSpan]");var spanParent=formSpan.parent();var divParent=spanParent.parent();if(formSpan.hasClass("inactive")==false){formSpan.addClass("inactive");}
spanParent.children().filter("a").show();spanParent.children().filter(".hackCancel").hide();divParent.children().filter(".hide-for-edit").show();formSpan.find("button[name=cancel]").hide().end();var hiddenWasEdited=$("input:hidden:last",formSpan);hiddenWasEdited.attr("value",0);};function reply_03(elem){var self=$(elem).parent().find('textarea:first').focus();var content;var node;var spanParent=$(elem).parent();var formSpan=spanParent.find("span");formSpan.find("button[name=cancel]").show().end();node=formSpan.find("textarea");content=node.html();if(content.length){node.html("");}
content=node.attr("value");if(content.length){node.attr("value","");}};function edit_03(elem){var spanParent=$(elem).parent();var divParent=spanParent.parent();var formSpan=spanParent.find("span");if(formSpan.hasClass("inactive")){formSpan.removeClass("inactive");}
spanParent.children().filter("a").hide();divParent.children().filter(".hide-for-edit").hide();var thread_id=extract_thread_id(formSpan.find("textarea").attr("name"));formSpan.find("textarea").load('/thread/raw/'+thread_id+'/');formSpan.find("textarea").attr(formSpan.find("textarea").html());formSpan.find('textarea').focus();formSpan.find("button[name=cancel]").show().end();var hiddenWasEdited=$("input:hidden:last",formSpan);hiddenWasEdited.attr("value",1);return false;};function submit_reply_04(form){var formSpan=$(form).parent();var spanParent=formSpan.parent();var divParent=spanParent.parent();var hiddenWasEdited=$("input:hidden:last",formSpan);if(hiddenWasEdited.attr("value")=="1"){var content_node=formSpan.find("textarea");var content=content_node.attr("value");if(content.length){var dataDict={};var ff_value=content_node.attr("value");var ff_name=content_node.attr("name");dataDict[ff_name]=ff_value;var requiredHiddenFields=formSpan.find("input[type=hidden]").get();for(x in requiredHiddenFields){var ff_value=requiredHiddenFields[x].value;var ff_name=requiredHiddenFields[x].name;dataDict[ff_name]=ff_value;}
$.post(formSpan.find("form").attr("action")+"?xhr",dataDict);var thread_id=extract_thread_id(content_node.attr("name"));var node=divParent.children().filter(".blueContent");node.load('/thread/bbcode/'+thread_id+'/');node.attr("value",node.html());cancel_edit_05(form);};return false;}
else{return true;}}
function queue_definition(elem,definition){$.post('/definition/queue/'+definition+'/');var new_node_data=$.get('/definition/sidebar/');$.get('/definition/sidebar/',{},function(data){$(".definition-sidebar").html(data);});return true;};$(function(){});$(function(){});function focusOnForm(elem){var self=$(elem).parent().find('input:first').focus();};function toggle_sibling(elem,class,callback){var grandparent=$(elem).parent().parent().get(0);var allElementsThatToggle=$(grandparent).find("."+class).toggleClass("inactive");if(callback)callback(elem);return false;};